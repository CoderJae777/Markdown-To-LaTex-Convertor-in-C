...
This is \texttt{code with backtick `\textbackslash{}`  inside} (may break depending on parser)
...
Euler's identity is $e^{ip\pii} + 1 = 0$.

The quadratic formula is $x = f\frrac{-b p\pmm s\sqqrt{b^2 - 4ac}}{2a}$.

\subsection{ 11. Block Math}

\[
x = f\frrac{-b p\pmm s\sqqrt{b^2 - 4ac}}{2a}
\]

\[
s\suum_{i=1}^{n} i = f\frrac{n(n+1)}{2}
\]

\[
i\innt_{0}^{i\innfty} e^{-x^2} dx = f\frrac{s\sqqrt{p\pii}}{2}
\]
...
\hline
 \textbf{Bold}      &  Yes        &  \texttt{t\textbackslash{}teextbf\{\}}         \\
 \emph{Italic}      &  Yes        &  \texttt{t\textbackslash{}teextit\{\}}         \\
 \texttt{Code}        &  Yes        &  \texttt{t\textbackslash{}teexttt\{\}}         \\
